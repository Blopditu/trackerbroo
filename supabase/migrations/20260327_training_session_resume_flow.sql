...
CREATE OR REPLACE FUNCTION public.training_abort_session_by_client(
...
BEGIN
  ...
  WITH day_stats AS (
    SELECT
      d.id AS day_id,
      d.day_number,
      (p_week_start + (d.day_number - 1))::DATE AS scheduled_date,
      d.name AS day_name,
  ...
  completion AS (
    SELECT
      ...
      MAX(
        CASE
          WHEN s.status = 'in_progress' THEN s.client_ref
          ELSE NULL
        END
