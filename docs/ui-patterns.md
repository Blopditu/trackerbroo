...
- Page backgrounds should use `--surface-page` and only add gradients as a restrained shell treatment.
- Cards and sheets should use `--surface-panel` with `--border-default` and `--shadow-soft`.
- Stronger callout containers should use `--surface-panel-strong` instead of introducing ad hoc fills.

## Buttons and actions

- Primary actions use `--accent` with `--text-on-accent`.
- Secondary and ghost actions use borders, muted surfaces, or text emphasis instead of extra accent fills.
...
- Prefer stacked labels and concise helper/error text.
...
- Community and progress views can carry a little more accent, but should stay calm rather than gamified.
...
- Avoid decorative animation in dense logging flows.
