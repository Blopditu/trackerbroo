  ...
  template: `
    <div class="hub-tabs" role="group" aria-label="Session Hub Tabs">
      ...
      <button
        ...
        (click)="tabChange.emit('exercises')"
      >
        Übungen
      </button>
      <button
    ...
    @if (tab() === 'plans') {
      <div class="sheet-stack">
        <button mat-flat-button type="button" class="action-btn" (click)="startPlanBuilder.emit()">
        ...
        @for (plan of plans(); track plan.id) {
          <article class="list-card sheet-card">
            ...
            <button
              ...
              (click)="activatePlan.emit(plan.id)"
            >
        ...
        <section class="list-card sheet-card filter-card">
          ...
          <mat-form-field class="m3-field" appearance="outline" subscriptSizing="dynamic">
            ...
            <mat-select
              ...
              (valueChange)="equipmentFilterChange.emit($event)"
            >
          ...
          <mat-form-field class="m3-field" appearance="outline" subscriptSizing="dynamic">
            ...
            <mat-select
              ...
              (valueChange)="muscleFilterChange.emit($event)"
            >
              <mat-option value="">Alle</mat-option>
          ...
          <p class="muted">
            {{ filteredExerciseLibrary().length }} von {{ exercisesCount() }} Übungen
          </p>
        </section>
    ...
    @if (tab() === 'help') {
      <div class="sheet-stack">
        <article class="list-card sheet-card text-only">
          ...
          <p class="muted">
            Vorherige Sätze werden automatisch übernommen, damit du nur bestätigen oder anpassen
            musst.
          </p>
        </article>
        <article class="list-card sheet-card text-only">
          ...
          <p class="muted">
            Während des Workouts stehen nur aktueller Satz, Historie und Abschluss im Fokus.
          </p>
        </article>
        <article class="list-card sheet-card text-only">
          <strong>Offline First</strong>
          <p class="muted">
            Alle Schreibaktionen werden offline lokal gespeichert und automatisch synchronisiert.
          </p>
        </article>
      </div>
    }
