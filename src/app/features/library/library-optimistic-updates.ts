...
export function buildIngredientMutationDraft(
  ...
  const nextIngredients = editingIngredient
    ? currentIngredients.map(item => (item.id === editingIngredient.id ? optimisticIngredient : item))
...
export function buildMealMutationDraft(
  ...
  const filteredItems = mealForm.getRawValue().items
  ...
  const optimisticMealItems = [
    ...currentMealItems.filter(item => item.meal_id !== optimisticMealId && item.meal_id !== editingMeal?.id),
    ...filteredItems.map(item => ({ meal_id: optimisticMealId, ingredient_id: item.ingredient_id, grams: item.grams }))
  ];
...
export function remapMealItems(items: MealItem[], fromMealId: string, toMealId: string): MealItem[] {
  return items.map(item =>
    item.meal_id === fromMealId ? { ...item, meal_id: toMealId } : item
  );
}

export function finalizeMealItems(currentMealItems: MealItem[], mealId: string, items: Array<{ ingredient_id: string; grams: number }>): MealItem[] {
  return [
    ...currentMealItems.filter(item => item.meal_id !== mealId),
    ...items.map(item => ({ meal_id: mealId, ingredient_id: item.ingredient_id, grams: item.grams }))
  ];
}
