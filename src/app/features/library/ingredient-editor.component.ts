...
@Component({
  ...
  template: `
    <h2 id="ingredient-modal-title" class="title-font">
    ...
    <form [formGroup]="form()" (ngSubmit)="save.emit()" class="stack-form">
      ...
      <mat-form-field class="m3-field" appearance="outline" subscriptSizing="dynamic">
        ...
        <mat-select
          ...
          (valueChange)="sourceTypeChange.emit()"
        >
      ...
      <button
        mat-flat-button
        type="button"
        class="action-btn tonal compact"
        [disabled]="!macroPasteText().trim()"
        (click)="applyMacroPaste.emit()"
      >
        Makros übernehmen
      </button>
      ...
      <button
        mat-flat-button
        type="button"
        class="action-btn ghost compact details-toggle"
