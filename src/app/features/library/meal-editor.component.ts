...
@Component({
  ...
  template: `
    <h2 id="meal-modal-title" class="title-font">{{ editing() ? 'Mahlzeit bearbeiten' : 'Mahlzeit hinzufügen' }}</h2>
      ...
      <div class="meal-items" formArrayName="items">
        @for (item of itemControls(); track $index; let index = $index) {
          <div class="meal-item" [formGroupName]="index">
            ...
            <button mat-flat-button type="button" class="action-btn danger compact" (click)="removeItem.emit(index)">Entfernen</button>
          </div>
        }
      </div>

      <p class="cost-preview">Geschätzte Mahlzeitenkosten: {{ draftMealCostLabel() }}</p>
      <button mat-flat-button type="button" class="action-btn tonal" (click)="addItem.emit()">+ Zutat hinzufügen</button>
