...
const templateRoot = path.join(repoRoot, 'starter', 'angular', 'files');
...
Bootstraps a fresh Angular project with the Tracker Broo starter guardrails:
...
async function updatePackageJson(targetRoot) {
  ...
  for (const dependency of copiedDevDependencies) {
    const version = sourcePackage.devDependencies?.[dependency];
    if (!version) {
      continue;
    }
